...
metadata <- data@meta.data[,c("Sample","Species","Collection","Individual","Replicate","Individual.Replicate","Stage","Cluster","AdHoc.Assign.median","OstAdHoc.Assign.5gene.median")]
...
ggplot(HC.var.melt, aes(x=species, y=value, fill=cluster)) +
  ...
  theme_classic() +
  theme(axis.text=element_text(size=10), axis.title=element_text(size=10), legend.text=element_text(size=10))
ggplot(HC.var.melt %>% filter(cluster %in% c("iPSC.c1","MSC.c1","Osteogenic.c1")), aes(x=species, y=value, fill=cluster)) +
  geom_boxplot(width=0.4) +
  xlab("Species") +
  ylab('log2 variance in gene expression') +
  scale_fill_manual(name="Cluster",
...
### SC: Clusters vs. Individual.Replicates
...
### SC: Ad Hoc Classifications vs. Species
...
### SC: Ad Hoc Classifications vs. Individuals
...
### SC: Ad Hoc Classifications vs. Individual.Replicates
...
### SC: Osteogenic Ad Hoc Classifications vs. Species 
...
# Calculate log2cpm means within species - mean decreases in ost (expt C), decreases in emb.ost, decreases in min.ost, and increases in mat.ost
HC.mean <- data.frame(matrix(nrow=dim(tmm)[1],ncol=0))
HC.mean["gene"] <- rownames(tmm)
HC.mean["Human_preosteoblast"] <- as.data.frame(apply(tmm[,which(groupFactor=="Human_preosteoblast" & dge$samples$Stage=="Time 2")],1, mean))
HC.mean["Human_osteoblast"] <- as.data.frame(apply(tmm[,which(groupFactor=="Human_osteoblast" & dge$samples$Stage=="Time 2")],1, mean))
HC.mean["Human_embedding.osteoblast"] <- as.data.frame(apply(tmm[,which(groupFactor=="Human_embedding.osteoblast" & dge$samples$Stage=="Time 2")],1, mean))
HC.mean["Human_mineralizing.osteoblast"] <- as.data.frame(apply(tmm[,which(groupFactor=="Human_mineralizing.osteoblast" & dge$samples$Stage=="Time 2")],1, mean))
HC.mean["Human_maturing.osteocyte"] <- as.data.frame(apply(tmm[,which(groupFactor=="Human_maturing.osteocyte" & dge$samples$Stage=="Time 2")],1, mean))
HC.mean["Chimp_preosteoblast"] <- as.data.frame(apply(tmm[,which(groupFactor=="Chimp_preosteoblast" & dge$samples$Stage=="Time 2")],1, mean))
HC.mean["Chimp_osteoblast"] <- as.data.frame(apply(tmm[,which(groupFactor=="Chimp_osteoblast" & dge$samples$Stage=="Time 2")],1, mean))
HC.mean["Chimp_embedding.osteoblast"] <- as.data.frame(apply(tmm[,which(groupFactor=="Chimp_embedding.osteoblast" & dge$samples$Stage=="Time 2")],1, mean))
HC.mean["Chimp_mineralizing.osteoblast"] <- as.data.frame(apply(tmm[,which(groupFactor=="Chimp_mineralizing.osteoblast" & dge$samples$Stage=="Time 2")],1, mean))
HC.mean["Chimp_maturing.osteocyte"] <- as.data.frame(apply(tmm[,which(groupFactor=="Chimp_maturing.osteocyte" & dge$samples$Stage=="Time 2")],1, mean))
HC.mean.melt <- melt(HC.mean,
                     id.vars="gene",
                     measure.vars=c("Human_preosteoblast","Human_osteoblast","Human_embedding.osteoblast","Human_mineralizing.osteoblast","Human_maturing.osteocyte",
                                    "Chimp_preosteoblast","Chimp_osteoblast","Chimp_embedding.osteoblast","Chimp_mineralizing.osteoblast","Chimp_maturing.osteocyte"))
...
ggplot(HC.mean.melt, aes(x=species, y=value, fill=ostadhoc)) +
  ...
  theme_classic() +
  theme(axis.text=element_text(size=10), axis.title=element_text(size=10), legend.text=element_text(size=10))
t.test(HC.mean$Human_preosteoblast,HC.mean$Human_osteoblast)                     #p=1.1e-14
t.test(HC.mean$Human_osteoblast,HC.mean$Human_embedding.osteoblast)              #p<2.2e-16
t.test(HC.mean$Human_embedding.osteoblast,HC.mean$Human_mineralizing.osteoblast) #p<2.2e-16
t.test(HC.mean$Human_mineralizing.osteoblast,HC.mean$Human_maturing.osteocyte)   #p<2.2e-16
t.test(HC.mean$Chimp_preosteoblast,HC.mean$Chimp_osteoblast)                     #p=0.33010
t.test(HC.mean$Chimp_osteoblast,HC.mean$Chimp_embedding.osteoblast)              #p<2.2e-16
t.test(HC.mean$Chimp_embedding.osteoblast,HC.mean$Chimp_mineralizing.osteoblast) #p=0.03444
t.test(HC.mean$Chimp_mineralizing.osteoblast,HC.mean$Chimp_maturing.osteocyte)   #p=1.4e-15

# Calculate log2cpm variances within species - variance increases in ost (expt C), increases in emb.ost, increases in min.ost (expt C), and decreases in mat.ost
HC.var <- data.frame(matrix(nrow=dim(tmm)[1],ncol=0))
HC.var["gene"] <- rownames(tmm)
HC.var["Human_preosteoblast"] <- as.data.frame(apply(tmm[,which(groupFactor=="Human_preosteoblast" & dge$samples$Stage=="Time 2")],1, var))
HC.var["Human_osteoblast"] <- as.data.frame(apply(tmm[,which(groupFactor=="Human_osteoblast" & dge$samples$Stage=="Time 2")],1, var))
HC.var["Human_embedding.osteoblast"] <- as.data.frame(apply(tmm[,which(groupFactor=="Human_embedding.osteoblast" & dge$samples$Stage=="Time 2")],1, var))
HC.var["Human_mineralizing.osteoblast"] <- as.data.frame(apply(tmm[,which(groupFactor=="Human_mineralizing.osteoblast" & dge$samples$Stage=="Time 2")],1, var))
HC.var["Human_maturing.osteocyte"] <- as.data.frame(apply(tmm[,which(groupFactor=="Human_maturing.osteocyte" & dge$samples$Stage=="Time 2")],1, var))
HC.var["Chimp_preosteoblast"] <- as.data.frame(apply(tmm[,which(groupFactor=="Chimp_preosteoblast" & dge$samples$Stage=="Time 2")],1, var))
HC.var["Chimp_osteoblast"] <- as.data.frame(apply(tmm[,which(groupFactor=="Chimp_osteoblast" & dge$samples$Stage=="Time 2")],1, var))
HC.var["Chimp_embedding.osteoblast"] <- as.data.frame(apply(tmm[,which(groupFactor=="Chimp_embedding.osteoblast" & dge$samples$Stage=="Time 2")],1, var))
HC.var["Chimp_mineralizing.osteoblast"] <- as.data.frame(apply(tmm[,which(groupFactor=="Chimp_mineralizing.osteoblast" & dge$samples$Stage=="Time 2")],1, var))
HC.var["Chimp_maturing.osteocyte"] <- as.data.frame(apply(tmm[,which(groupFactor=="Chimp_maturing.osteocyte" & dge$samples$Stage=="Time 2")],1, var))
HC.var.melt <- melt(HC.var,
                    id.vars="gene",
                    measure.vars=c("Human_preosteoblast","Human_osteoblast","Human_embedding.osteoblast","Human_mineralizing.osteoblast","Human_maturing.osteocyte",
                                   "Chimp_preosteoblast","Chimp_osteoblast","Chimp_embedding.osteoblast","Chimp_mineralizing.osteoblast","Chimp_maturing.osteocyte"))
...
t.test(HC.var$Human_preosteoblast,HC.var$Human_osteoblast)                     #p<2.2e-16
t.test(HC.var$Human_osteoblast,HC.var$Human_embedding.osteoblast)              #p<2.2e-16
t.test(HC.var$Human_embedding.osteoblast,HC.var$Human_mineralizing.osteoblast) #p=0.08523
t.test(HC.var$Human_mineralizing.osteoblast,HC.var$Human_maturing.osteocyte)   #p<2.2e-16
t.test(HC.var$Chimp_preosteoblast,HC.var$Chimp_osteoblast)                     #p=0.28990
t.test(HC.var$Chimp_osteoblast,HC.var$Chimp_embedding.osteoblast)              #p<2.2e-16
t.test(HC.var$Chimp_embedding.osteoblast,HC.var$Chimp_mineralizing.osteoblast) #p=0.00997
t.test(HC.var$Chimp_mineralizing.osteoblast,HC.var$Chimp_maturing.osteocyte)   #p<2.2e-16

```

### SC: Osteogenic Ad Hoc Classifications vs. Individuals
...
### SC: Osteogenic Ad Hoc Classifications vs. Individual.Replicates
...
### Pseudobulk: Ad Hoc Classifications vs. Species
...
### Pseudobulk: Osteogenic Ad Hoc Classifications vs. Species
