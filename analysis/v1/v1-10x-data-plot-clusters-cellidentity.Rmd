...
compare.obl <- runSEURATnegbinom(subset(data,subset=OstAdHoc.Assign=="osteoblast"), genes.no.ribo)
compare.obl  <- FindMarkers(subset(data,subset=OstAdHoc.Assign=="osteoblast"),
                            assay="RNA",
                            slot="counts",
                            ident.1="Human",
...
#latent.vars=c("Collection","nCount_RNA","Phase")(counts): 436/2036 --------------------- (USE THIS MODEL FOR NOW)
...
compare.obl <- runSEURATpoisson(subset(data,subset=OstAdHoc.Assign=="osteoblast"), genes.no.ribo)
compare.obl  <- FindMarkers(subset(data,subset=OstAdHoc.Assign=="osteoblast"),
                            assay="RNA",
                            slot="counts",
                            ident.1="Human",
...
compare.obl <- runSEURATmast(subset(data,subset=OstAdHoc.Assign=="osteoblast"), genes.no.ribo)
compare.obl  <- FindMarkers(subset(data,subset=OstAdHoc.Assign=="osteoblast"),
                            assay="RNA",
                            slot="data",
                            ident.1="Human",
...
table(compare.obl$p_val_adj<0.05)
head(compare.obl, n=5)
#latent.vars="nCount_RNA"(counts): 615/1465
...
funcEnrichCC <- function(data, markers) {
  genes <- structure(rep(1, length(rownames(data@assays$RNA@data))), names=rownames(data@assays$RNA@data))
  x=1
  while (x <= length(rownames(markers))) {
    genes[which(names(genes)==rownames(markers)[x])] <- markers$p_val_adj[x]
  ...
  go_data <- new("topGOdata", ontology="CC", allGenes=genes, geneSel=function(allScore){return(allScore < 0.01)},
                 nodeSize=5, annotationFun=annFUN.org, mapping="org.Hs.eg.db", ID="symbol")
  go_test <- runTest(go_data, algorithm="weight01", statistic="fisher")
  go_table <- GenTable(go_data, weightFisher=go_test, orderBy="weightFisher", ranksOf="weightFisher", topNodes=sum(score(go_test) < .01))
  return(go_table)
}
funcEnrichBP <- function(data, markers) {
  genes <- structure(rep(1, length(rownames(data@assays$RNA@data))), names=rownames(data@assays$RNA@data))
  x=1
  while (x <= length(rownames(markers))) {
    genes[which(names(genes)==rownames(markers)[x])] <- markers$p_val_adj[x]
...
intersect3way <- function(group1, group2, group3) {
  group.tot <- intersect(rownames(group1)[which(group1$p_val_adj<0.05)],
                         intersect(rownames(group2)[which(group2$p_val_adj<0.05)],
                                   rownames(group3)[which(group3$p_val_adj<0.05)]))
  return(group.tot)
}
setdiff3way <- function(target, diff1, diff2) {
  group.tot <- setdiff(rownames(target)[which(target$p_val_adj<0.05)],
                       c(rownames(diff1)[which(diff1$p_val_adj<0.05)],
                         rownames(diff2)[which(diff2$p_val_adj<0.05)]))
  return(group.tot)
}
df3way <- function(labels, group1, group2, group3, group.tot, group1.tot, group2.tot, group3.tot) {
  DE.df <- data.frame(cell=labels,
                      totalDEgenes=c(length(group1$gene[which(group1$p_val_adj<0.05)]),
                                     length(group2$gene[which(group2$p_val_adj<0.05)]),
                                     length(group3$gene[which(group3$p_val_adj<0.05)])),
                      Other=c((length(group1$gene[which(group1$p_val_adj<0.05)])-length(group1.tot)-length(group.tot)),
                              (length(group2$gene[which(group2$p_val_adj<0.05)])-length(group2.tot)-length(group.tot)),
                              (length(group3$gene[which(group3$p_val_adj<0.05)])-length(group3.tot)-length(group.tot))),
                      Shared=c(rep(length(group.tot),3)),
                      Unique=c(length(group1.tot),
                               length(group2.tot),
                               length(group3.tot)))
  DE.df$cell <- factor(DE.df$cell, levels=labels)
  return(DE.df)
}

intersect7way <- function(group1, group2, group3, group4, group5, group6, group7) {
  group.tot <- intersect(rownames(group1)[which(group1$p_val_adj<0.05)],
                         intersect(rownames(group2)[which(group2$p_val_adj<0.05)],
                                   intersect(rownames(group3)[which(group3$p_val_adj<0.05)],
                                             intersect(rownames(group4)[which(group4$p_val_adj<0.05)],
                                                       intersect(rownames(group5)[which(group5$p_val_adj<0.05)],
                                                                 intersect(rownames(group6)[which(group6$p_val_adj<0.05)],
                                                                           rownames(group7)[which(group7$p_val_adj<0.05)]))))))
  return(group.tot)
}
setdiff7way <- function(target, diff1, diff2, diff3, diff4, diff5, diff6) {
  group.tot <- setdiff(rownames(target)[which(target$p_val_adj<0.05)],
                       c(rownames(diff1)[which(diff1$p_val_adj<0.05)],
                         rownames(diff2)[which(diff2$p_val_adj<0.05)],
                         rownames(diff3)[which(diff3$p_val_adj<0.05)],
                         rownames(diff4)[which(diff4$p_val_adj<0.05)],
                         rownames(diff5)[which(diff5$p_val_adj<0.05)],
                         rownames(diff6)[which(diff6$p_val_adj<0.05)]))
  return(group.tot)
}
df7way <- function(labels2, group1, group2, group3, group4, group5, group6, group7,
                   group.tot, group1.tot, group2.tot, group3.tot, group4.tot, group5.tot, group6.tot, group7.tot) {
  DE.df <- data.frame(cell=labels2,
                      totalDEgenes=c(length(group1$gene[which(group1$p_val_adj<0.05)]),
                                     length(group2$gene[which(group2$p_val_adj<0.05)]),
                                     length(group3$gene[which(group3$p_val_adj<0.05)]),
                                     length(group4$gene[which(group4$p_val_adj<0.05)]),
                                     length(group5$gene[which(group5$p_val_adj<0.05)]),
                                     length(group6$gene[which(group6$p_val_adj<0.05)]),
                                     length(group7$gene[which(group7$p_val_adj<0.05)])),
                      Other=c((length(group1$gene[which(group1$p_val_adj<0.05)])-length(group1.tot)-length(group.tot)),
                              (length(group2$gene[which(group2$p_val_adj<0.05)])-length(group2.tot)-length(group.tot)),
                              (length(group3$gene[which(group3$p_val_adj<0.05)])-length(group3.tot)-length(group.tot)),
                              (length(group4$gene[which(group4$p_val_adj<0.05)])-length(group4.tot)-length(group.tot)),
                              (length(group5$gene[which(group5$p_val_adj<0.05)])-length(group5.tot)-length(group.tot)),
                              (length(group6$gene[which(group6$p_val_adj<0.05)])-length(group6.tot)-length(group.tot)),
                              (length(group7$gene[which(group7$p_val_adj<0.05)])-length(group7.tot)-length(group.tot))),
                      Shared=c(rep(length(group.tot),7)),
                      Unique=c(length(group1.tot),
                               length(group2.tot),
                               length(group3.tot),
                               length(group4.tot),
                               ...
                               length(group6.tot),
                               length(group7.tot)))
  DE.df$cell <- factor(DE.df$cell, levels=labels2)
  return(DE.df)
}

#Find differentially expressed genes using differentiation stage at collection
labels=c("iPSC","MSC","Osteoblast")
compare.1 <- readRDS(paste0(dir,"/human-chimp-skeletal-scRNA/data/cellranger-data-full/de.edger-compare.ips.rds"))
compare.2 <- readRDS(paste0(dir,"/human-chimp-skeletal-scRNA/data/cellranger-data-full/de.edger-compare.msc.rds"))
compare.3 <- readRDS(paste0(dir,"/human-chimp-skeletal-scRNA/data/cellranger-data-full/de.edger-compare.ost.rds"))
#Find differentially expressed genes using cluster definitions
labels=c("iPSC","MSC","Osteoblast")
labels2=c("iPSC.c1","iPSC.c2","iPSC.c3","MSC.c1","Osteo.c1","Osteo.c2","Osteo.c3")
compare.1 <- readRDS(paste0(dir,"/human-chimp-skeletal-scRNA/data/cellranger-data-full/de.edger-compare.i.rds"))
compare.2 <- readRDS(paste0(dir,"/human-chimp-skeletal-scRNA/data/cellranger-data-full/de.edger-compare.m.rds"))
compare.3 <- readRDS(paste0(dir,"/human-chimp-skeletal-scRNA/data/cellranger-data-full/de.edger-compare.o.rds"))
compare.1a <- readRDS(paste0(dir,"/human-chimp-skeletal-scRNA/data/cellranger-data-full/de.edger-compare.i1.rds"))
compare.1b <- readRDS(paste0(dir,"/human-chimp-skeletal-scRNA/data/cellranger-data-full/de.edger-compare.i2.rds"))
compare.1c <- readRDS(paste0(dir,"/human-chimp-skeletal-scRNA/data/cellranger-data-full/de.edger-compare.i3.rds"))
compare.3a <- readRDS(paste0(dir,"/human-chimp-skeletal-scRNA/data/cellranger-data-full/de.edger-compare.o1.rds"))
compare.3b <- readRDS(paste0(dir,"/human-chimp-skeletal-scRNA/data/cellranger-data-full/de.edger-compare.o2.rds"))
compare.3c <- readRDS(paste0(dir,"/human-chimp-skeletal-scRNA/data/cellranger-data-full/de.edger-compare.o3.rds"))
#Find differentially expressed genes using ad hoc definitions
labels=c("iPSC","MSC","Osteoblast")
compare.1 <- readRDS(paste0(dir,"/human-chimp-skeletal-scRNA/data/cellranger-data-full/de.edger-compare.hoci.rds"))
compare.2 <- readRDS(paste0(dir,"/human-chimp-skeletal-scRNA/data/cellranger-data-full/de.edger-compare.hocm.rds"))
compare.3 <- readRDS(paste0(dir,"/human-chimp-skeletal-scRNA/data/cellranger-data-full/de.edger-compare.hoco.rds"))
#Find differentially expressed genes using osteogenic ad hoc definitions
labels=c("preosteoblast","osteoblast","embedding osteoblast")
compare.1 <- readRDS(paste0(dir,"/human-chimp-skeletal-scRNA/data/cellranger-data-full/de.edger-compare.pre.rds"))
compare.2 <- readRDS(paste0(dir,"/human-chimp-skeletal-scRNA/data/cellranger-data-full/de.edger-compare.obl.rds"))
compare.3 <- readRDS(paste0(dir,"/human-chimp-skeletal-scRNA/data/cellranger-data-full/de.edger-compare.emb.rds"))

#Restructure data if necessary
compare.1 <- compare.1$table
compare.2 <- compare.2$table
compare.3 <- compare.3$table
compare.1a <- compare.1a$table
compare.1b <- compare.1b$table
compare.1c <- compare.1c$table
compare.3a <- compare.3a$table
compare.3b <- compare.3b$table
compare.3c <- compare.3c$table
#Rename columns if necessary
colnames(compare.1)[5] <- "p_val_adj"
colnames(compare.2)[5] <- "p_val_adj"
colnames(compare.3)[5] <- "p_val_adj"
colnames(compare.1a)[5] <- "p_val_adj"
colnames(compare.1b)[5] <- "p_val_adj"
colnames(compare.1c)[5] <- "p_val_adj"
colnames(compare.3a)[5] <- "p_val_adj"
colnames(compare.3b)[5] <- "p_val_adj"
colnames(compare.3c)[5] <- "p_val_adj"
#Reorder data if necessary
compare.1 <- compare.1[order(-compare.1$logFC, compare.1$p_val_adj),]
compare.2 <- compare.2[order(-compare.2$logFC, compare.2$p_val_adj),]
compare.3 <- compare.3[order(-compare.3$logFC, compare.3$p_val_adj),]
compare.1a <- compare.1a[order(-compare.1a$logFC, compare.1a$p_val_adj),]
compare.1b <- compare.1b[order(-compare.1b$logFC, compare.1b$p_val_adj),]
compare.1c <- compare.1c[order(-compare.1c$logFC, compare.1c$p_val_adj),]
compare.3a <- compare.3a[order(-compare.3a$logFC, compare.3a$p_val_adj),]
compare.3b <- compare.3b[order(-compare.3b$logFC, compare.3b$p_val_adj),]
compare.3c <- compare.3c[order(-compare.3c$logFC, compare.3c$p_val_adj),]


#View results of 3 cell types
paste0("All Genes Examined: ",dim(compare.1)[1],", ",dim(compare.2)[1],", ",dim(compare.3)[1])
paste0("DE Genes: ",table(compare.1$p_val_adj<0.05)[2],", ",table(compare.2$p_val_adj<0.05)[2],", ",table(compare.3$p_val_adj<0.05)[2])
head(compare.1, n=5)
head(compare.2, n=5)
head(compare.3, n=5)
compare.1$gene <- rownames(compare.1)
compare.2$gene <- rownames(compare.2)
compare.3$gene <- rownames(compare.3)
head(funcEnrichCC(data,compare.1), n=5)
head(funcEnrichCC(data,compare.2), n=5)
head(funcEnrichCC(data,compare.3), n=5)
compare.tot <- intersect3way(compare.1, compare.2, compare.3)
compare.1.tot <- setdiff3way(compare.1, compare.2, compare.3)
compare.2.tot <- setdiff3way(compare.2, compare.1, compare.3)
compare.3.tot <- setdiff3way(compare.3, compare.1, compare.2)
paste0("Shared DE Genes: ",length(compare.tot))
paste0("Unique DE Genes: ",length(compare.1.tot),", ",length(compare.2.tot),", ",length(compare.3.tot))
head(compare.1[which(rownames(compare.1)%in%compare.tot),], n=5)
head(compare.1[which(rownames(compare.1)%in%compare.1.tot),], n=5)
head(compare.2[which(rownames(compare.2)%in%compare.2.tot),], n=5)
head(compare.3[which(rownames(compare.3)%in%compare.3.tot),], n=5)
head(funcEnrichCC(data,compare.1[which(rownames(compare.1)%in%compare.tot),]), n=5)
head(funcEnrichCC(data,compare.1[which(rownames(compare.1)%in%compare.1.tot),]), n=5)
head(funcEnrichCC(data,compare.2[which(rownames(compare.2)%in%compare.2.tot),]), n=5)
head(funcEnrichCC(data,compare.3[which(rownames(compare.3)%in%compare.3.tot),]), n=5)
head(funcEnrichBP(data,compare.1[which(rownames(compare.1)%in%compare.tot),]), n=5)
head(funcEnrichBP(data,compare.1[which(rownames(compare.1)%in%compare.1.tot),]), n=5)
head(funcEnrichBP(data,compare.2[which(rownames(compare.2)%in%compare.2.tot),]), n=5)
head(funcEnrichBP(data,compare.3[which(rownames(compare.3)%in%compare.3.tot),]), n=5)
DE.df <- df3way(labels=labels,compare.1,compare.2,compare.3,compare.tot,compare.1.tot,compare.2.tot,compare.3.tot)
ggplot(DE.df, aes(x=cell, y=totalDEgenes)) +
  geom_bar(stat="identity") +
  labs(x="",y="Number of DE Genes") +
  theme_classic() +
...
ggplot(DE.df[4:12,], aes(x=cell, y=value, fill=variable)) +
  ...
  theme_classic() +
  theme(axis.text=element_text(size=12), axis.title=element_text(size=12), legend.text=element_text(size=12), legend.title=element_text(size=0))
all.genes <- merge(compare.1, merge(compare.2, compare.3, by="gene", all.x=TRUE, all.y=TRUE), by="gene", all.x=TRUE, all.y=TRUE)
all.genes$compare.1 <- 0
all.genes$compare.2 <- 0
all.genes$compare.3 <- 0
i=1
while(i <= length(all.genes$gene)) {
  if(is.na(all.genes$p_val_adj[i])==FALSE & all.genes$p_val_adj[i] < 0.05) {all.genes$compare.1[i] <- 1}
  if(is.na(all.genes$p_val_adj.x[i])==FALSE & all.genes$p_val_adj.x[i] < 0.05) {all.genes$compare.2[i] <- 1}
  if(is.na(all.genes$p_val_adj.y[i])==FALSE & all.genes$p_val_adj.y[i] < 0.05) {all.genes$compare.3[i] <- 1}
  i=i+1
}
upset(all.genes[,17:19], nintersects=NA, empty.intersections="on", keep.order=TRUE, order.by="degree", decreasing=TRUE)

#View results of 7 cell types
paste0("All Genes Examined: ",dim(compare.1a)[1],", ",dim(compare.1b)[1],", ",dim(compare.1c)[1],", ",
       dim(compare.2)[1],", ",dim(compare.3a)[1],", ",dim(compare.3b)[1],", ",dim(compare.3c)[1])
paste0("DE Genes: ",table(compare.1a$p_val_adj<0.05)[2],", ",table(compare.1b$p_val_adj<0.05)[2],", ",
       table(compare.1c$p_val_adj<0.05)[2],", ",table(compare.2$p_val_adj<0.05)[2],", ",
       table(compare.3a$p_val_adj<0.05)[2],", ",table(compare.3b$p_val_adj<0.05)[2],", ",
       table(compare.3c$p_val_adj<0.05)[2])
head(compare.1a, n=5)
head(compare.1b, n=5)
head(compare.1c, n=5)
head(compare.2, n=5)
...
head(compare.3b, n=5)
head(compare.3c, n=5)
compare.1a$gene <- rownames(compare.1a)
compare.1b$gene <- rownames(compare.1b)
compare.1c$gene <- rownames(compare.1c)
compare.2$gene <- rownames(compare.2)
compare.3a$gene <- rownames(compare.3a)
compare.3b$gene <- rownames(compare.3b)
compare.3c$gene <- rownames(compare.3c)
head(funcEnrichCC(data,compare.1a), n=5)
head(funcEnrichCC(data,compare.1b), n=5)
head(funcEnrichCC(data,compare.1c), n=5)
head(funcEnrichCC(data,compare.2), n=5)
head(funcEnrichCC(data,compare.3a), n=5)
head(funcEnrichCC(data,compare.3b), n=5)
head(funcEnrichCC(data,compare.3c), n=5)
compare.tot <- intersect7way(compare.1a, compare.1b, compare.1c, compare.2, compare.3a, compare.3b, compare.3c)
compare.1a.tot <- setdiff7way(compare.1a, compare.1b, compare.1c, compare.2, compare.3a, compare.3b, compare.3c)
compare.1b.tot <- setdiff7way(compare.1b, compare.1a, compare.1c, compare.2, compare.3a, compare.3b, compare.3c)
compare.1c.tot <- setdiff7way(compare.1c, compare.1a, compare.1b, compare.2, compare.3a, compare.3b, compare.3c)
compare.2.tot <- setdiff7way(compare.2, compare.1a, compare.1b, compare.1c, compare.3a, compare.3b, compare.3c)
compare.3a.tot <- setdiff7way(compare.3a, compare.1a, compare.1b, compare.1c, compare.2, compare.3b, compare.3c)
compare.3b.tot <- setdiff7way(compare.3b, compare.1a, compare.1b, compare.1c, compare.2, compare.3a, compare.3c)
compare.3c.tot <- setdiff7way(compare.3c, compare.1a, compare.1b, compare.1c, compare.2, compare.3a, compare.3b)
paste0("Shared DE Genes: ",length(compare.tot))
paste0("Unique DE Genes: ",length(compare.1a.tot),", ",length(compare.1b.tot),", ",length(compare.1c.tot),", ",
       length(compare.2.tot),", ",length(compare.3a.tot),", ",length(compare.3b.tot),", ",length(compare.3c.tot))
head(compare.1a[which(rownames(compare.1a)%in%compare.tot),], n=5)
head(compare.1a[which(rownames(compare.1a)%in%compare.1a.tot),], n=5)
head(compare.1b[which(rownames(compare.1b)%in%compare.1b.tot),], n=5)
head(compare.1c[which(rownames(compare.1c)%in%compare.1c.tot),], n=5)
head(compare.2[which(rownames(compare.2)%in%compare.2.tot),], n=5)
head(compare.3a[which(rownames(compare.3a)%in%compare.3a.tot),], n=5)
head(compare.3b[which(rownames(compare.3b)%in%compare.3b.tot),], n=5)
head(compare.3c[which(rownames(compare.3c)%in%compare.3c.tot),], n=5)
head(funcEnrichCC(data,compare.1a[which(rownames(compare.1a)%in%compare.tot),]), n=5)
head(funcEnrichCC(data,compare.1a[which(rownames(compare.1a)%in%compare.1a.tot),]), n=5)
head(funcEnrichCC(data,compare.1b[which(rownames(compare.1b)%in%compare.1b.tot),]), n=5)
head(funcEnrichCC(data,compare.1c[which(rownames(compare.1c)%in%compare.1c.tot),]), n=5)
head(funcEnrichCC(data,compare.2[which(rownames(compare.2)%in%compare.2.tot),]), n=5)
head(funcEnrichCC(data,compare.3a[which(rownames(compare.3a)%in%compare.3a.tot),]), n=5)
head(funcEnrichCC(data,compare.3b[which(rownames(compare.3b)%in%compare.3b.tot),]), n=5)
head(funcEnrichCC(data,compare.3c[which(rownames(compare.3c)%in%compare.3c.tot),]), n=5)
head(funcEnrichBP(data,compare.1a[which(rownames(compare.1a)%in%compare.tot),]), n=5)
head(funcEnrichBP(data,compare.1a[which(rownames(compare.1a)%in%compare.1a.tot),]), n=5)
head(funcEnrichBP(data,compare.1b[which(rownames(compare.1b)%in%compare.1b.tot),]), n=5)
head(funcEnrichBP(data,compare.1c[which(rownames(compare.1c)%in%compare.1c.tot),]), n=5)
head(funcEnrichBP(data,compare.2[which(rownames(compare.2)%in%compare.2.tot),]), n=5)
head(funcEnrichBP(data,compare.3a[which(rownames(compare.3a)%in%compare.3a.tot),]), n=5)
head(funcEnrichBP(data,compare.3b[which(rownames(compare.3b)%in%compare.3b.tot),]), n=5)
head(funcEnrichBP(data,compare.3c[which(rownames(compare.3c)%in%compare.3c.tot),]), n=5)
DE.df <- df7way(labels=labels2,compare.1a,compare.1b,compare.1c,compare.2,compare.3a,compare.3b,compare.3c,
                compare.tot,compare.1a.tot,compare.1b.tot,compare.1c.tot,compare.2.tot,compare.3a.tot,compare.3b.tot,compare.3c.tot)
ggplot(DE.df, aes(x=cell, y=totalDEgenes)) +
  geom_bar(stat="identity") +
  labs(x="",y="Number of DE Genes") +
  theme_classic() +
...
ggplot(DE.df[8:28,], aes(x=cell, y=value, fill=variable)) +
  ...
  theme_classic() +
  theme(axis.text=element_text(size=12), axis.title=element_text(size=12), legend.text=element_text(size=12), legend.title=element_text(size=0))
all.genes <- merge(compare.1a,
                   merge(compare.1b,
                         merge(compare.1c,
                               merge(compare.2,
                                     merge(compare.3a,
                                           merge(compare.3b, compare.3c, by="gene", all.x=TRUE, all.y=TRUE),
                                           by="gene", all.x=TRUE, all.y=TRUE),
                                     by="gene", all.x=TRUE, all.y=TRUE),
                               by="gene", all.x=TRUE, all.y=TRUE),
                         by="gene", all.x=TRUE, all.y=TRUE),
                   by="gene", all.x=TRUE, all.y=TRUE)
all.genes$compare.1a <- 0
all.genes$compare.1b <- 0
all.genes$compare.1c <- 0
all.genes$compare.2 <- 0
all.genes$compare.3a <- 0
all.genes$compare.3b <- 0
all.genes$compare.3c <- 0
i=1
while(i <= length(all.genes$gene)) {
  if(is.na(all.genes$p_val_adj[i])==FALSE & all.genes$p_val_adj[i] < 0.05) {all.genes$compare.1a[i] <- 1}
  if(is.na(all.genes$p_val_adj.x[i])==FALSE & all.genes$p_val_adj.x[i] < 0.05) {all.genes$compare.1b[i] <- 1}
  if(is.na(all.genes$p_val_adj.y[i])==FALSE & all.genes$p_val_adj.y[i] < 0.05) {all.genes$compare.1c[i] <- 1}
  if(is.na(all.genes$p_val_adj.x.2[i])==FALSE & all.genes$p_val_adj.x.2[i] < 0.05) {all.genes$compare.2[i] <- 1}
  if(is.na(all.genes$p_val_adj.y.2[i])==FALSE & all.genes$p_val_adj.y.2[i] < 0.05) {all.genes$compare.3a[i] <- 1}
  if(is.na(all.genes$p_val_adj.x.1[i])==FALSE & all.genes$p_val_adj.x.1[i] < 0.05) {all.genes$compare.3b[i] <- 1}
  if(is.na(all.genes$p_val_adj.y.1[i])==FALSE & all.genes$p_val_adj.y.1[i] < 0.05) {all.genes$compare.3c[i] <- 1}
  i=i+1
}
upset(all.genes[,37:43], nsets=7, nintersects=NA, empty.intersections="on", keep.order=TRUE, order.by="degree", decreasing=TRUE)
...
```{r UTF1 and TAC3 expression}

#POTENTIAL MARKERS OF DIFFERENTIATION POTENTIAL
