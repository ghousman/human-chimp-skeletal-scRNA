...
obj.ipsc <- lapply(X=obj.ipsc, FUN=function(x) {
    x <- ScaleData(x, features=obj.ipsc.features, verbose=FALSE, vars.to.regress=c("nCount_RNA","nFeature_RNA"))
...
saveRDS(int.ipsc, file=paste0(dir,"/human-chimp-skeletal-scRNA/data/cellranger-data-full/data.filter.regB.indv-cell.log.10k.int.ipsc.rds"))
...
obj.msc <- lapply(X=obj.msc, FUN=function(x) {
    x <- ScaleData(x, features=obj.msc.features, verbose=FALSE, vars.to.regress=c("nCount_RNA","nFeature_RNA"))
...
saveRDS(int.msc, file=paste0(dir,"/human-chimp-skeletal-scRNA/data/cellranger-data-full/data.filter.regB.indv-cell.log.10k.int.msc.rds"))
rm(obj.msc,int.msc)
...
obj.osteo <- lapply(X=obj.osteo, FUN=function(x) {
    x <- ScaleData(x, features=obj.osteo.features, verbose=FALSE, vars.to.regress=c("nCount_RNA","nFeature_RNA"))
...
int.osteo <- ScaleData(int.osteo, verbose=FALSE, vars.to.regress=c("nCount_RNA","nFeature_RNA"))
...
saveRDS(int.osteo, file=paste0(dir,"/human-chimp-skeletal-scRNA/data/cellranger-data-full/data.filter.regB.indv-cell.log.10k.int.osteo.rds"))
