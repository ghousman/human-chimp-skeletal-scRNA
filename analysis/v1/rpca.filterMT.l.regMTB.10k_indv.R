...
objects <- lapply(X=objects, FUN=function(x) {
    x <- ScaleData(x, features=objects.features, verbose=FALSE, vars.to.regress=c("nCount_RNA","nFeature_RNA","percent.mt"))
...
integrate <- ScaleData(integrate, verbose=FALSE, vars.to.regress=c("nCount_RNA","nFeature_RNA","percent.mt"))
...
#Save data
saveRDS(integrate, file=paste0(dir,"/human-chimp-skeletal-scRNA/data/cellranger-data-full/data.filterMT.regMTB.indv.log.10k.int.rds"))
