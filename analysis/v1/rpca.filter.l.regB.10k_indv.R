...
#Load objects
objects <- readRDS(paste0(dir,"/human-chimp-skeletal-scRNA/data/cellranger-data-full/data.filter.indv.log.rds"))
...
objects.features <- SelectIntegrationFeatures(object.list=objects, nfeatures=10000)
reference_dataset <- c(2,9) #make H1-r2 and C1-r2 the references
objects <- lapply(X=objects, FUN=function(x) {
    x <- ScaleData(x, features=objects.features, verbose=FALSE, vars.to.regress=c("nCount_RNA","nFeature_RNA"))
...
integrate <- ScaleData(integrate, verbose=FALSE, vars.to.regress=c("nCount_RNA","nFeature_RNA"))
...
print(ndim)
integrate <- RunUMAP(integrate, dims=1:ndim)

#Save data
saveRDS(integrate, file=paste0(dir,"/human-chimp-skeletal-scRNA/data/cellranger-data-full/data.filter.regB.indv.log.10k.int.rds"))
