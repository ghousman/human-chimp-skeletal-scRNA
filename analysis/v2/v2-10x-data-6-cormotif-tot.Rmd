...
### Cormotif: Stages only - 2 time point comparisons

```{r TEMPORARY}

# Compile group labels of each sample
groupFactor <- factor(sapply(dge$samples$Stage,function(x)sub(" ","",x)),
                      levels=c("Time0","Time1","Time2"))
groupID <- as.numeric(groupFactor)

# Compile comparison pattern
#(condiditions to compare = time0, time1, and time2 for each species)
compareID <- data.frame(c1 = c(1, 2),
                        c2 = c(2, 3))
...
gene.prob <- cormotifData$bestmotif$p.post
rownames(gene.prob) <- rownames(tmm)
colnames(gene.prob) <- c("t0-t1","t1-t2")
head(gene.prob)

# Examine genes from each motif
prob.motif1  <- rownames(gene.prob[(gene.prob[,1]<0.5 & gene.prob[,2]<0.5),])
length(prob.motif1)
head(prob.motif1)
#boxplot(tmm[prob.motif1[1],]~groupFactor)

prob.motif2  <- rownames(gene.prob[(gene.prob[,1]>0.5 & gene.prob[,2]<0.5),])
length(prob.motif2)
head(prob.motif2)
#boxplot(tmm[prob.motif2[1],]~groupFactor)
