...
#Load data
scrna <- "./data/cellranger-data-full/data.filter.log.indv-cell.int19k.reg-t2.rds"
data <- readRDS(scrna)
...
#labelBatch2 <- as.factor(data@meta.data$Sample[which(rownames(data@meta.data) %in% rownames(counts))])
#countsBC2 <- BatchCorrectedCounts(counts, labelBatch2, use_parallel=FALSE)
#labelBatch3 <- as.factor(data@meta.data$Species[which(rownames(data@meta.data) %in% rownames(counts))])
#countsBC3 <- BatchCorrectedCounts(counts, labelBatch3, use_parallel=FALSE)
#head(colSums(ifelse(counts==countsBC1, 0, 1) != 0))
#head(colSums(ifelse(counts==countsBC2, 0, 1) != 0))
#head(colSums(ifelse(counts==countsBC3, 0, 1) != 0))
#head(colSums(ifelse(countsBC1==countsBC2, 0, 1) != 0))
#head(colSums(ifelse(countsBC1==countsBC3, 0, 1) != 0))
#head(colSums(ifelse(countsBC2==countsBC3, 0, 1) != 0))
#rm(data)

#Fit fastTopics model and save data
fit <- fit_poisson_nmf(counts, k=3, numiter=100)
saveRDS(fit, file="./data/gom-data/topics3.log.indv-cell.int19k.reg-t2.rds")
#fit <- fit_poisson_nmf(countsBC1, k=3, numiter=100)
#saveRDS(fit, file="./data/gom-data/topics3.log.indv-cell.int19k.reg-t2.bc1.rds")
fit <- fit_poisson_nmf(countsBC2, k=3, numiter=100)
saveRDS(fit, file="./data/gom-data/topics3.log.indv-cell.int19k.reg-t2.bc2.rds")
#fit <- fit_poisson_nmf(countsBC3, k=3, numiter=100)
#saveRDS(fit, file="./data/gom-data/topics3.log.indv-cell.int19k.reg-t2.bc3.rds")

#Fit GoM model (OLD METHOD) and save data
fit <- CountClust::FitGoM(counts, K=7, tol=1, control=list(tmax=100))
saveRDS(fit, file="./data/gom-data/gom7.log.indv-cell.int19k.reg-t2.rds")
...
sbatch ./fastTopics.sh 3 t2
sbatch ./fastTopics.sh 4 t2
sbatch ./fastTopics.sh 5 t2
sbatch ./fastTopics.sh 6 t2
sbatch ./fastTopics.sh 7 t2
sbatch ./fastTopics.sh 8 t2
sbatch ./fastTopics.sh 9 t2
sbatch ./fastTopics.sh 10 t2
sbatch ./fastTopics.sh 15 t2
sbatch ./fastTopics.sh 20 t2
sbatch ./fastTopics.sh 30 t2

sbatch ./fastTopicsSpp.sh 3 t2
sbatch ./fastTopicsSpp.sh 7 t2
sbatch ./fastTopicsSpp.sh 10 t2
sbatch ./fastTopicsSpp.sh 15 t2
sbatch ./fastTopicsSpp.sh 20 t2
sbatch ./fastTopicsSpp.sh 30 t2
...
fit <- readRDS(paste0("./data/gom-data/topics",numclust,".log.indv-cell.int19k.reg-t2.rds"))
#fit <- readRDS(paste0("./data/gom-data/topics",numclust,".log.indv-cell.int19k.reg-t2.bc1.rds"))
#fit <- readRDS(paste0("./data/gom-data/topics",numclust,".log.indv-cell.int19k.reg-t2.bc2.rds"))
#fit <- readRDS(paste0("./data/gom-data/topics",numclust,".log.indv-cell.int19k.reg-t2.bc3.rds"))
...
#fastTopics BIC
x=c(3,4,5,6,7,8,9,10,15,20,30)
y=c(521487311,507922510,495232478,486388734,478418694,474067758,469719239,467162020,454878366,448704431,442474271)
plot(x,y,xlab="Topics (k)",ylab="BIC")
lines(x[order(x)],y[order(x)],xlim=range(x),ylim=range(y),pch=16)

#fastTopics.Human BIC
x=c(3,7,10,15,20,30)
y=c(229821623,215821206,212393848,208487051,206794048,205458593)
plot(x,y,xlab="Topics (k)",ylab="BIC")
lines(x[order(x)],y[order(x)],xlim=range(x),ylim=range(y),pch=16)

#fastTopics.Chimp BIC
x=c(3,7,10,15,20,30)
y=c(265188479,246079015,241270004,237446404,235454947,233695411)
plot(x,y,xlab="Topics (k)",ylab="BIC")
lines(x[order(x)],y[order(x)],xlim=range(x),ylim=range(y),pch=16)
...
colclust <- c("dodgerblue","grey30","grey70")
#colclust <- c("dodgerblue4","dodgerblue1","grey30")
#colclust <- c("dodgerblue4","dodgerblue1","grey70")
...
#Create labels
labels <- data@meta.data$Sample[which(rownames(data@meta.data) %in% rownames(omega))]
annotation1 <- data.frame(sample_id=paste0("X", c(1:NROW(omega))), tissue_label=as.factor(labels))
celltype <- labels
celltype[celltype %in% c("C1-O","C1-O-r2","C2-O","C3-O","C4-O","C5-O","C6-O")] <- "Chimp"
celltype[celltype %in% c("H1-O","H1-O-r2","H2-O","H3-O","H4-O","H5-O","H6-O")] <- "Human"
...
genes[[1]][,1:10]
saveRDS(genes, file=paste0("./data/gom-data/topics",numclust,".100genes.log.indv-cell.int19k.reg-t2.rds"))
#saveRDS(genes, file=paste0("./data/gom-data/topics",numclust,".100genes.log.indv-cell.int19k.reg-t2.bc1.rds"))
#saveRDS(genes, file=paste0("./data/gom-data/topics",numclust,".100genes.log.indv-cell.int19k.reg-t2.bc2.rds"))
#saveRDS(genes, file=paste0("./data/gom-data/topics",numclust,".100genes.log.indv-cell.int19k.reg-t2.bc3.rds"))
...
#Read GoM data
fit <- readRDS(paste0("./data/gom-data/gom",numclust,".log.indv-cell.int19k.reg-t2.rds"))
...
genes[[1]][,1:10]
saveRDS(genes, file=paste0("./data/gom-data/gom",numclust,".100genes.log.indv-cell.int19k.reg-t2.rds"))
...
#Rescale log normalized gene expression to range from 0 to 1
z.tot.genes <- as.data.frame(apply(l.tot.genes[,1:28], MARGIN=2, FUN=function(X)(X-min(X))/diff(range(X))))
...
chk.thres <- function(data, alpl, col1a1, col1a2, runx2, bglap, capg, phex, mepe) {
  assign <- ""
  if(data[1] > alpl)   { assign <- paste(assign,"ALPL") }
  if(data[2] > col1a1) { assign <- paste(assign,"COL1A1") }
  if(data[3] > col1a2) { assign <- paste(assign,"COL1A2") }
...
test.genes <- as.data.frame(t(as.matrix(data@assays$integrated@scale.data[genes[c(19,17:18,9:11,13:14)],])))
test.genes$assign <- apply(test.genes, 1, chk.thres, alpl=0, col1a1=0, col1a2=0, runx2=0, bglap=0, capg=0, phex=0, mepe=0) #threshold = 0
test <- test.genes %>% group_by(assign) %>% summarise(total=n()) %>% dplyr::mutate(freq=total/sum(total))
...
hoc.assign <- function(assignments) {
  ...
  while(i <= length(rownames(assignments))) {
    ...
    #   (assignments$CAPG[i] == 1 & assignments$PHEX[i] == 1 & assignments$MEPE[i] == 1)) {assignments$assign[i] <- "Osteogenic"}
    i=i+1
  }
 return(as.data.frame(assignments$assign, row.names=rownames(assignments)))
}
...
#       (assignments$RUNX2[i] == 1 & assignments$BGLAP[i] == 1) |
#       (assignments$BGLAP[i] == 1 & assignments$CAPG[i] == 1 & assignments$PHEX[i] == 1) |
#       #(assignments$BGLAP[i] == 1) |
#       #(assignments$CAPG[i] == 1) |
#       #(assignments$PHEX[i] == 1) |
#       #(assignments$MEPE[i] == 1) |
#       (assignments$PHEX[i] == 1 & assignments$MEPE[i] == 1)) {assignments$assign[i] <- "Osteogenic"}
#    i=i+1
#  }
# return(as.data.frame(assignments$assign, row.names=rownames(assignments)))
#}
...
#assign osteogenic cell sub-type labels based on ad hoc cutoffs (using 5 genes) (scaled integrated data, q1)
...
#assign osteogenic cell sub-type labels based on ad hoc cutoffs (using 5 genes) (scaled integrated data, q1 for general genes / median for osteo specific genes)
...
#assign osteogenic cell sub-type labels based on ad hoc cutoffs (using 5 genes) (scaled integrated data, median for general genes / q1 for osteo specific genes)
