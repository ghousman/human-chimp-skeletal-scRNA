...
runEDGER <- function(dataSub, cell.assign, cell.subset, genes.no.mito.ribo, filter.arg, filter.param) {
  ...
  #narrow list of DE genes to those with at least a fold-change of 1.2
  qlf <- glmTreat(fit, coef=dim(design)[2], lfc=log2(1.2))

  #assess output
  tt <- topTags(qlf, n=Inf, adjust.method="BY", p.value=1)
  print("All DE Genes")
  print(dim(tt$table))
  print(table(tt$table$FDR<0.01))
  print(summary(decideTests(qlf, adjust.method="BY", p.value=0.01)))
...
for (i in subset.list) {
  ...
  for (j in i) {
    ...
    saveRDS(edgr.total, file=paste0("./../data/de-data/DEedgeR.fc.",name,".",assign,".rds"))
