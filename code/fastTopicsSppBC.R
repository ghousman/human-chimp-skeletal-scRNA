...
saveRDS(fit, file=paste0("../data/gom-data/topics",clustnum,".human.data.filterC.log.indv-cell.intNo0.reg-",data.source,".bc1.rds"))


#Run analyses for chimp data
print("Loading chimp data")

#Load data
scrna <- paste0("../data/cellranger-data-full/data.filterC.log.indv-cell.intNo0.reg-",data.source,".rds")
data <- readRDS(scrna)
data <- subset(data, subset=Species=="Chimp")
...
#Fit fastTopics model and save data
fit <- fit_poisson_nmf(countsBC1, k=clustnum, numiter=100)
saveRDS(fit, file=paste0("../data/gom-data/topics",clustnum,".chimp.data.filterC.log.indv-cell.intNo0.reg-",data.source,".bc1.rds"))
